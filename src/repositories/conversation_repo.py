...
class ConversationRepository:
    ...
    async def delete_for_phone(self, vacancy_id: uuid.UUID, phone: str):
        ...
        conv_ids = await self.pool.fetch(
            "SELECT id FROM agents.pre_screening_sessions WHERE vacancy_id = $1 AND candidate_phone = $2",
            vacancy_id, phone
        )
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    for row in conv_ids:
                        ...
                        await conn.execute(
                            "DELETE FROM agents.pre_screening_session_turns WHERE conversation_id = $1",
                            conv_id
                        )
                    ...
                    await conn.execute(
                        """
                        DELETE FROM agents.pre_screening_sessions
    ...
    async def delete_all(self):
        ...
        await self.pool.execute("DELETE FROM agents.pre_screening_sessions")
