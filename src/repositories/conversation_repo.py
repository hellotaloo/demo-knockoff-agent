...
class ConversationRepository:
    ...
    async def list_for_vacancy(
        ...
        total = await self.pool.fetchval(
            f"SELECT COUNT(*) FROM ats.pre_screening_conversations WHERE {where_clause}",
            *params
        )
    ...
    async def delete_for_phone(self, vacancy_id: uuid.UUID, phone: str):
        ...
        conv_ids = await self.pool.fetch(
            "SELECT id FROM ats.pre_screening_conversations WHERE vacancy_id = $1 AND candidate_phone = $2",
            vacancy_id, phone
        )
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    for row in conv_ids:
                        ...
                        await conn.execute(
                            "DELETE FROM ats.pre_screening_messages WHERE conversation_id = $1",
                            conv_id
                        )
                    ...
                    await conn.execute(
                        """
                        DELETE FROM ats.pre_screening_conversations
    ...
    async def delete_all(self):
        ...
        await self.pool.execute("DELETE FROM ats.pre_screening_conversations")
